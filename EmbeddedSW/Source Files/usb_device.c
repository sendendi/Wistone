...
********************************************************************
 ...
 Change History:
  ...
  2.8    Several changes to the way control transfers get processed,
   ...
   2.9   Fixed an issue with STALL handling behaviour on non-EP0 endpoints, for 
...
usb_config.h), then the application firmware is responsible for calling the 
...
#if defined(USB_DEVICE_DISABLE_DTS_CHECKING) // YL no such MACROs in v2011-10-05
...
// YL 10.9 ...
// commented because USB_SUPPORT_OTG is not defined
/*
...
// ... YL 10.9
	...
	// YL 10.9 ...
	// commented because USB_POLLING is not defined
	/*
	...
	*/
	...
	// YL 10.9 ...
	// commented because USB_SUPPORT_OTG isn't defined
	/*
    #ifdef  USB_SUPPORT_OTG
        //If ID Pin Changed State
        if (USBIDIF && USBIDIE)
...
USB_HANDLE USBTransferOnePacket(BYTE ep,BYTE dir,BYTE* data,BYTE len)
{
    ...
    handle->CNT = len;
    handle->STAT.Val &= _DTSMASK;
    handle->STAT.Val |= _USIE | (_DTSEN & _DTS_CHECKING_ENABLED);	// YL v2011-10-05 _USIE | _DTSEN
...
void USBStallEndpoint(BYTE ep, BYTE dir)
{
    ...
    if(ep == 0)
    {
        ...
        pBDTEntryEP0OutNext->STAT.Val = _USIE|_DAT0|(_DTSEN & _DTS_CHECKING_ENABLED)|_BSTALL;	// YL v2011-10-05 _USIE|_DAT0|_DTSEN|_BSTALL
...
void USBCtrlEPAllowStatusStage(void)
{
    ...
    if(USBStatusStageEnabledFlag1 == FALSE)
    {
        ...
        if(USBStatusStageEnabledFlag2 == FALSE)
        {
            ...
            {
                pBDTEntryIn[0]->CNT = 0;
                pBDTEntryIn[0]->STAT.Val = _USIE|_DAT1|(_DTSEN & _DTS_CHECKING_ENABLED);    // YL v2011-10-05  _USIE|_DAT1|_DTSEN    
            }
...
void USBCtrlEPAllowDataStage(void)
{
    ...
    if(controlTransferState == CTRL_TRF_RX) //(<setup><out><out>...<out><in>)
    {
        ...
        pBDTEntryEP0OutNext->STAT.Val = _USIE|_DAT1|(_DTSEN & _DTS_CHECKING_ENABLED); 	// YL v2011-10-05 _USIE|_DAT1|_DTSEN
    }   
...
static void USBCtrlTrfRxService(void)
{
    ...
    if(outPipes[0].wCount.Val > 0)
    {
        ...
        {
            pBDTEntryEP0OutNext->STAT.Val = _USIE|_DAT0|(_DTSEN & _DTS_CHECKING_ENABLED); // YL v2011-10-05 _USIE|_DAT0|_DTSEN
        }
    }
    else
    {
...
static void USBStallHandler(void)
{
    ...
    if(U1EP0bits.EPSTALL == 1)
    {
        ...
        if((pBDTEntryEP0OutCurrent->STAT.Val == _USIE) && (pBDTEntryIn[0]->STAT.Val == (_USIE|_BSTALL)))
        {
            ...
            pBDTEntryEP0OutCurrent->STAT.Val = _USIE|_DAT0|(_DTSEN & _DTS_CHECKING_ENABLED)|_BSTALL;	// YL v2011-10-05 _USIE|_DAT0|_DTSEN|_BSTALL
        }//end if
...
static void USBCtrlEPService(void)
{
    ...
    #if defined(USB_ENABLE_STATUS_STAGE_TIMEOUTS) // YL USB_ENABLE_STATUS_STAGE_TIMEOUTS is defined (should it be undefined?) 
...
static void USBCtrlTrfSetupHandler(void)
{
    ...
    USB_NONSTANDARD_EP0_REQUEST_HANDLER(EVENT_EP0_REQUEST,0,0);  //Check for USB device class specific requests // YL v2011-10-05 USB_DISABLE_NONSTANDARD_EP0_REQUEST_HANDLER(EVENT_EP0_REQUEST,0,0)
    ...
    //   process the request.  In this case, the default behaviour will be to
...
static void USBCtrlTrfInHandler(void)
{
    ...
    if(controlTransferState == CTRL_TRF_TX)
    {
        ...
        else
        {
            ...
            {
                pBDTEntryIn[0]->STAT.Val = _USIE|_DAT0|(_DTSEN & _DTS_CHECKING_ENABLED); // YL v2011-10-05 _USIE|_DAT0|_DTSEN 
            }
        }//end if(...)else
    }
