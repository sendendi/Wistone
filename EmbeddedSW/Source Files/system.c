...
/***** GLOBAL VARIABLES: ******************************************************/
char			*g_version = "1.05.0"; 				// program version number. to be incremented each new release - Yossi's update - according to "3.2.13 network recovery update" folder from dropbox
...
void init_all(void)
{
	...
	init_temp_sensor();		
	init_flash();
	init_accmtr(); 	//TODO!!! - if the SensorI board is not assembled the SW gets stuck here. need to fix this.
...
#ifdef WISDOM_STONE
	err_clear();
	//display_welcome(); 	//YL 26.8 - TODO - add description of what is connected and of what is not 
#endif //#ifdef WISDOM_STONE
	return;
}
...
int handle_system(int sub_cmd)
{
	...
	switch (sub_cmd) {
		...
		case SUB_CMD_SPOWER:
			handle_set_power_enable(); //YL 17.10 - meanwhile commented to avoid damage to the system
			break;
...
void handle_get_power_status(void)	//YL 13.8 - is it possible to check whether the battery/accelerometer are connected (like FLASH crad detect?...)
{
	...
	m_write("\r\n\tBattery status: ");
	m_write(uint_to_str(g_vbat_level));	//YL 11.11 instead of disp_num_to_term
	m_write(" - ");
...
void write_ver()
{
	...
	if (res > MAX_SN) 
		...
		serial = res;
	m_write(" HW Version: ");
	m_write(uint_to_str((unsigned int)serial));	//YL 11.11 instead of disp_num_to_term
